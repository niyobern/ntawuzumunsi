...
def get_links(db: Session = Depends(get_db), current_user : schemas.User =Depends(oauth2.get_current_user)):
    ...
    if role == "no_role":
        ...
        paths = ["/kitchen/products", "/kitchen/requests", "/products", "/products/sales", "/stock", "/stock/purchases", "/kitchen/commande", "/cash", "/users"]
        links = ["Kitchen Products", "Material Requests", "Products", "Sales", "Stock", "Purchases", "Commande", "Cash", "Users"]
        return {"links": links, "paths": paths}
    else:
